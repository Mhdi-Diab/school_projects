...
Rectangle::Rectangle(Window *win, int x, int y) :  Render(win){
	this->topLeftX = (x - 2 >= 0 ? x - 2 : (x - 1 >= 0 ? x - 1 : x));
	this->topLeftY = (y - 2 >= 0 ? y - 2 : (y - 1 >= 0 ? y - 1 : y));
	this->bottomRightX = (x + 2 < SIZE ? x + 2 : (x + 1 < SIZE ? x + 1 : x));
	this->bottomRightY = (y + 2 < SIZE ? y + 2 : (y + 1 < SIZE ? y + 1 : y));
...
Rectangle::Rectangle(Rectangle &rhs) : Render(rhs.getWindow()) {
...
void Rectangle::RenderRectangle(void) {
	this->r.x = this->topLeftX * 70 + PADDING_X;
    this->r.y = this->topLeftY * 70 + PADDING_Y;
    this->r.w = this->width * 70;
    this->r.h = this->height * 70;
	SDL_SetRenderDrawColor( this->win_->getRenderer(), this->col[0], this->col[1], this->col[2], this->col[3] );
   	SDL_RenderFillRect( this->win_->getRenderer(), &this->r );
}
...
void Rectangle::resize(int x, int y, t_color color) {
...
bool Rectangle::hasPieceInside(Rectangle *rect, std::map<std::pair<int,int>, t_color> pieces) {
...
bool Rectangle::shouldIncreaseBottomX(int x) {
	return (x > this->bottomRightX - 2 && this->bottomRightX < SIZE);
}

bool Rectangle::shouldIncreaseBottomY(int y) {
	return (y > this->bottomRightY - 2 && this->bottomRightY < SIZE);
}
